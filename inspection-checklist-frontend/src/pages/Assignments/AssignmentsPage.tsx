...
const assignmentSchema = z.object({
  ...
  frequency: z.enum(frequencyOptions).default('weekly'),
  start_due_at: z
    .string()
          ...
          <table className="min-w-full divide-y divide-slate-200 text-sm">
            ...
            <tbody className="divide-y divide-slate-100 bg-white">
              {assignmentRows.length === 0 && (
                <tr>
                  <td colSpan={canStartInspection ? 6 : 5} className="px-3 py-6 text-center text-slate-500">
...
const AssignmentCreateCard = () => {
  ...
  const onSubmit = async (values: AssignmentFormValues) => {
    const template = templateOptions.find((item) => item.id === values.templateId)
