...
Action owners: UserRole.action_owner (app/models/entities.py (line 13)) represents the workforce that actually fixes corrective actions. They have read/write access only to the actions assigned to them, can upload/download attachments for those actions (app/services/files.py (line 63) + app/routers/files.py (line 23)), and they receive an email alert (app/templates/email/action_assigned.html) whenever a new action is assigned. The front-end locks their navigation to /actions and /actions/search, defaulting to the “My actions” scope (inspection-checklist-frontend/src/pages/Actions/ActionsList.tsx). Admins/reviewers can reassign actions via the modal (inspection-checklist-frontend/src/pages/Actions/ActionDetailsModal.tsx) and can still see org-wide metrics, while inspectors only see actions tied to the inspections they created.
